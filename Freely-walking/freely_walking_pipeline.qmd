...
1. Set the `MACHINE_ROLE` environment variable (see above)
2. Verify Python is installed at the path specified in `config.py`
3. Verify MATLAB is installed and on PATH
4. Verify the network drive is accessible: `dir \\prfs.hhmi.org\reiserlab\oaky-cokey`
5. Install the `watchdog` Python package (acquisition machine only)
6. Generate batch files: `python generate_batch_files.py`

**Processing machine additionally:**

- Create local data directories if they do not exist:

```cmd
mkdir %PROJECT_ROOT%\DATA\00_unprocessed
mkdir %PROJECT_ROOT%\DATA\01_tracked
mkdir %PROJECT_ROOT%\DATA\02_processed
